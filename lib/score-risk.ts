...
export async function scoreRisk(companyName: string, report: any): Promise<RiskScore> {
  ...
  try {
    ...
    if (!response.ok) { console.warn(`scoreRisk API error ${response.status}`); return fallback }
...
function inferFallback(report: any): RiskScore {
  ...
  const rep: RiskLevel = t.includes("scandal") || t.includes("backlash") ? "High" : t.includes("criticism") || t.includes("controversy") ? "Medium" : "Low"
  const reg: RiskLevel = t.includes("investigation") || t.includes("lawsuit") ? "High" : t.includes("regulatory") || t.includes("legal") ? "Medium" : "Low"
  const ops: RiskLevel = t.includes("attack") || t.includes("crisis") ? "High" : t.includes("leadership") || t.includes("disruption") ? "Medium" : "Low"
  const fin: RiskLevel = t.includes("catastrophic") || t.includes("bankruptcy") ? "High" : t.includes("volatile") || t.includes("earnings") ? "Medium" : "Low"
