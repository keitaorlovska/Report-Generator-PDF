...
export async function POST(req: Request) {
  try {
    ...
    const reports: { company: string; brief: any }[] = body.reports
      ? body.reports
      : [{ company: body.company, brief: body.report ?? body.brief }];
    ...
    for (const entry of reports) {
      ...
      const meaningful = whatChanged.filter((s: string) => !noMentionsRe.test(String(s)));
      const hasNoMentions = whatChanged.some((s: string) => noMentionsRe.test(String(s)));
